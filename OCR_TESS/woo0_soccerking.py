    ...
    for i, j in enumerate(df_list):
        ...
        if df_list[i][5] == 1 and df_list[i][11] != ' ':
            if (height // cnt) >= 10:
                ...
                result_list2.append([df_list[i - x_init][6], min(top_min), max(top_h_max)])
                top_min = []
                top_h_max = []
            top = df_list[i][7]
            height = df_list[i][9]
            cnt = 1
            top_min.append(df_list[i][7])
            top_h_max.append(df_list[i][9])
        else:
            x_init = df_list[i][5]
...
def cut_roi(img, axis_list, file_name):
    ...
    if height2 >= height_ratio and height_ratio != height2:
        resize_width = (width2 * height_ratio) // height2
        height2, width2 = height_ratio, resize_width

        show_re = cv2.resize(show_re , (width2 , height2))
    else:
        show_re = show_re
    cv2.imshow('result', show_re)
    cv2.waitKey(0)
...
def cut_roi_new(img, axis_list, file_name):
    ...
    #선긋기
    for i, j in enumerate(axis):
        result = cv2.line(img, (0, j[1]-mg), (width, j[1]-mg), (0, 0, 0), 1)
        result = cv2.line(img, (0, j[2]+mg), (width, j[2]+mg), (0, 255, 0), 5)
    show_re = result.copy()
    height_ratio = 1200
    ...
    for i, j in enumerate(axis):
        ...
        if j[1]-mg>0 or j[2]+mg <height:
            ...
            #img = cv2.imread('./c_4_3.jpg', 0)
            #img는 numpy 배열
            cut_height = j[2]+mg -(j[1]-mg)
            ...
            if HEIGHT > 50 :
                ...
                #    print(str(e))
                #resized = cv2.resize(result, (width_r,25), interpolation=cv2.INTER_AREA)
                background = cv2.resize(background[-1], (WIDTH,50), interpolation=cv2.INTER_AREA)
                ...
                cv2.imwrite('c_{}_{}'.format(i,file_name), background)
        # Result
            else :
                ratio_h = float(25/HEIGHT)
                width_r= int(ratio_h*WIDTH)
                ...
                for k in range(resized_h):
                    for l in range(resized_w):
                        background[y_offset + k][x_offset+l] = resized[k][l]
                cv2.imwrite('c_{}_{}'.format(i,file_name), letter)

    #img = cv2.imread('./c_2_4.jpg', 0)



        #cut_img.append(j[1]-5:j[2]+5)
...
def main():
    ...
    cut_roi_new(img = save_img, axis_list = topNheight_list, file_name=filename)
